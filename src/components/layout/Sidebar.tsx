...
const Sidebar: React.FC<SidebarProps> = ({ toggle, menuOpen, setMenuOpen, onToggle }) => {
  ...
  return (
    <aside 
      className={`w-[70%] sm:w-[75%] h-screen bg-[#32324D] transition-all duration-300 ease-in-out rounded-r-2xl fixed left-0 top-0 z-50 ${toggle ?  'md:w-[20%]' : 'md:-translate-x-0 md:w-[12%] lg:w-[9%]  -translate-x-[100%]'}`}>
      ...
      <div className={`flex flex-col h-full text-white text-sm font-semibold transition-all duration-300 ${toggle ? '' : ''}`}>
        ...
        <div className={`flex-1 overflow-y-auto scrollbar-hidden flex flex-col ${toggle ? '' : 'items-center'}`}>
          ...
          {/* profile */}
          <div className={`flex items-center gap-4 px-3 py-4 ${!toggle ? 'flex-row md:flex-col' : ''}`}>
          ...
          <div className='flex justify-center px-3 text-left flex-col space-y-3 mb-6 py-6 border-b-2 border-gray-400'>
            ...
            <div>
              ...
              >
                ...
                <p className={`md:text-[12px] lg:text-[15px] ${toggle ? 'flex' : 'hidden'}`}>Food Menu</p>
              </motion.button>
            ...
            <div>
              ...
              >
                ...
                <p className={`md:text-[12px] lg:text-[15px] ${toggle ? 'flex' : 'hidden'}`}>Order History</p>
              </motion.button>
            </div>
            ...
            <div>
              ...
              >
                ...
                <p className={`md:text-[12px] lg:text-[15px] ${toggle ? 'flex' : 'hidden'}`}>Location</p>
              </motion.button>
            </div>
          </div>

          {/* General */}
          <div className='flex justify-center px-3 text-left flex-col space-y-3'>
            ...
            <div>
              <motion.button 
                ...
                <p className={`md:text-[12px] lg:text-[15px] ${toggle ? 'flex' : 'hidden'}`}>My Rewards</p>
              </motion.button>
            </div>
            ...
            <div>
              <motion.button 
                ...
                <p className={`md:text-[12px] lg:text-[15px] ${toggle ? 'flex' : 'hidden'}`}>Help</p>
              </motion.button>
            </div>
          </div>
          ...
          <div className='flex justify-center px-3 text-left flex-col mt-10 space-y-3'>
            <div>
              <motion.button 
                ...
                <p className={`md:text-[12px] lg:text-[15px] ${toggle ? 'flex' : 'hidden'}`}>Logout</p>
              </motion.button>
            </div>
          </div>
        </div>

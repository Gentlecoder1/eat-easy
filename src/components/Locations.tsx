...
const Locations: React.FC = () => {
  ...
  return (
    <div
      className="bg-[#F7F7F7] w-full  min-h-screen"
      style={{
    ...
    >
      <div className={`py-[70px] max-w-6xl transition-all duration-300`}>
        <div className="bg-white max-w-[700px] mx-auto flex flex-col items-center p-6 space-y-5 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.10)]">
          <div className="flex items-center mb-5">
            <p className="lg:text-[28px] md:text-[20px] font-bold">
              Set your location
            </p>
          ...
          <div
            className="hidden flex-col items-center rounded-2xl bg-white shadow-2xl w-full"
            aria-label="input"
          >
            {Available.map((option, idx) => (
              <div
                key={idx}
                className="flex space-y-4 items-center rounded-2xl p-5 w-full"
              >
                <motion.div
                  whileTap={{ scale: 0.9 }}
